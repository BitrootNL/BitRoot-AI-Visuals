"""
``generate_plots.py`` — Regenerate all CCPlots matplotlib examples.

Usage: ``python generate_plots.py``

Configuration lives in ``CCPlots/config/`` (system package) and
``CCPlots/plot_configs/`` (per-example JSON files).  Colour palette is
sourced from ``CCPlots/config/bitroot.json``.
"""
import CCPlots
